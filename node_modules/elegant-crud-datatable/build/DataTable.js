...
} from "./DataTableColumn/DataTableSpecialColumn.js";
...
export class DataTable {
  ...
  buildTable() {
    ...
    return `<table class="table-auto w-full box-border border-separate ${this._dataTableStyles && this._dataTableStyles.table.join(" ")}">${header}${body}</table>`;
  }
  buildHead() {
    let base = `<thead><tr>`;
    this._columns.forEach((column) => {
      ...
      base += `<th class="${cellClasses.join(" ")}"><div class="table-header ${contentClasses.join(" ")}">`;
  ...
  buildBody() {
    ...
    this._data.forEach((data, index) => {
      ...
      if (data instanceof DataTableSubgroupSeparator) {
        ...
        row = this.newRow(objectData, data.rowClasses);
      }
      base += `${row}`;
    });
  ...
  newRow(rowData, rowClasses) {
    ...
    dataColumnsNames.forEach((columnName, index) => {
      ...
      row += `<td class="cell ${cellClasses.join(" ")} ${rowClass}"><p class="cell-input ${contentClasses.join(" ")} ${rowClass}" name="${columnName}-${rowId}">${data}</p></td>`;
    });
  ...
  buildCreatorRow(id) {
    ...
    this._columns.forEach((column, index) => {
      if (typeof column.header.name === "string") {
        ...
        row += `<td class="cell ${cellClasses.join(" ")} max-w-full"><p max="${column.body.newDataLength || 25}" contenteditable="true" id="${inputId}" name="${inputId}" class='${this._bgColorClass} cell-input ${contentClasses.join(" ")}' role="textbox"></p></td>`;
      }
    });
