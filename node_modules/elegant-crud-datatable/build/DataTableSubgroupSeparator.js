...
export class DataTableSubgroupSeparator {
  constructor(_id, _title, _data, _classes, _fnCreate, color, _canAdd) {
    this._id = _id;
    this._title = _title;
    this._data = _data;
    this._classes = _classes ? _classes : [];
    this._fnCreate = _fnCreate;
    this._canAdd = _canAdd || true;
    const colorsNames = Object.keys(separatorsColors).filter((color) => color !== "white");
    if (!color  || (!colorsNames.includes(color) && color !== "white")) {
  ...
  buildSeparator(columnsCount) {
    return `<tr>
    <td colspan="${columnsCount}" class="subgroup-description  ${this.separatorColor.bg} ${this.separatorColor.border} ${this._classes.join(" ")}">
      ${this._title}
...
export const separatorsColors = {
  ...
  white: {bg: "bg-white", border: "border-gray-100"}
};
