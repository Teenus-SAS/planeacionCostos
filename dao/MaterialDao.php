...
class MaterialDao
{
  ...
  {
    $this->db->connect();
    $query = "UPDATE `materiales` SET `unidad` = '" . $material->getUnit() . "', `costo` = '" . $material->getCost() . "' WHERE `materiales`.`id_materiales` = " . $material->getId() . " AND `materiales`.`empresas_id_empresa` = " . $material->getIdCompany();
    return $this->db->consult($query);
  }
