...
$.get("/app/products/api/get_products.php", (_products, status) => {
  ...
  $("#search-product").keyup(function () {
    let productsSought = products.filter(
      (product) =>
        product.ref.trim().toLowerCase().includes($(this).val()) ||
        product.name.trim().toLowerCase().includes($(this).val())
    );
    ...
    if (n > 0) {
      if (n > 1) {
        ...
        $.confirm({
          title: "Tezlik",
          content:
            "¿Desea Cotizar una Sola Cantidad para los productos Seleccionados?",
          buttons: {
            SI: function () {
                      ...
                      if (!quantity) {
                        ...
                        items.each(function (idx, item) {
                          ...
                          choice
                            .parent()
                            .append(
                              `<span class="pull-right quantity">${quantity}</span>`
                            );
                        });
                      }
                    },
              ...
              items.each(function (idx, item) {
                ...
                $.confirm({
                  ...
                  content:
                    ...
                    '<form class="formName">' +
                    '<div class="form-group">' +
                    `<label>Ingrese la cantidad para el producto ${choice
  ...
  $(".remove").click(function () {
    ...
    if (items.length == 0) {
      $.alert("Selecciona al menos un elemento para quitar", "Tezlik");
    }
  });
...
function loadProductsList(products) {
  ...
  products.forEach((product) => {
    $("#list1").append(
      `<a href="#" class="list-group-item"><span class="ref-product pull-left text-capitalize">${
        product.ref
      }</span>  <span class="product-name text-lowercase" title="${
        product.name
      }">${
        product.name.length > 30
          ? product.name.substring(0, 30) + " ..."
          : product.name
      }</span> <span class="id-product">${
        product.id
      }</span><input type="checkbox" class="pull-right"></a>`
    );
  });
  sizeList();
  selectAll();
    ...
    if ($(this).height() > 18) {
      while ($(this).height() > 18) {
        $(this).html(
          $(this)
            .html()
            .substring(0, $(this).html().length - 7) + " ..."
        );
      }
      $(this).html(
        $(this)
...
$("#select-lineas").change(function () {
  if ($(this).val() == "all") {
    ...
    $.get(
      "/app/products/api/get_products_line.php",
      {
        id: $(this).val(),
      },
      (data, status) => {
