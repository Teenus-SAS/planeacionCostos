...
function loadProductsGG() {
  ...
  $.get('api/get_products.php?expenses', (_products, status, xhr) => {
    ...
    if (status == 'success') {
      ...
      $('#inputRefGastos').html('<option disabled selected>Selecciona una Referencia</option>')
      $('#inputProductosGastos').html('<option disabled selected>Selecciona un Producto</option>')
      productsInExpenses = _products
      productsInExpenses.forEach((product) => {
        $('#inputRefGastos').append(`<option value="${product.id}">${product.ref}</option>`)
        $('#inputProductosGastos').append(`<option value="${product.id}">${product.name}</option>`)
      })

      $('#inputRefGastos').change(function () {
        let productSelected = productsInExpenses.filter(product => product.id == $(this).val())[0]
