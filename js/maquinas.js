...
function submitForm(e, option, maquina) {
  ...
  if (total == 0) {
    $.notify({
      icon: "nc-icon nc-bell-55", message: "Ingrese todos los <b>datos</b>"
    }, {
      type: 'primary', timer: 8000
    })
    return false;
  }
  ...
  if (valor_residual == 0) {
    $.notify({
      icon: "nc-icon nc-bell-55", message: "Ingrese el valor residual, este valor puede ser cero (0)"
    }, {
      type: 'primary', timer: 8000
    })
    return false;
  ...
  if (elById("inlineRadio1M").checked && !maquinaExists) {
    ...
    bootbox.confirm({
      ...
      message: `La Máquina <b>"${elById('input-maquinas').value}"</b> ya existe, ¿Desea actualizarla?`,
      buttons: {
        confirm: {
