...
$.get(
  ...
  (data, status) => {
    ...
    let chartProcess = $("#chartProcess").highcharts({
      ...
      tooltip: {
        pointFormat:
          "Porcentaje: <b>{point.percentage:.1f}%</b> <br> {series.name}: <b>{point.y:.2f} min</b>",
      },
      plotOptions: {
        pie: {
    ...
    Highcharts.chart("chartLaborCost", {
      ...
      tooltip: {
        ...
        pointFormatter: function () {
          return `Costo de mano de Obra: <b>$ ${Highcharts.numberFormat(
            this.y,
            "2",
            ","
          )} </b>`;
    ...
    if (data.expensesDescription == false) {
      ...
      expensesDataChart.push([
        ...
        data.expensesDescription["51"].value,
      ]);
      expensesDataChart.push([
        "Gastos Operacionales de Ventas",
        data.expensesDescription["52"].value,
      ]);
      expensesDataChart.push([
        "Gastos No operacionales",
        data.expensesDescription["53"].value,
      ]);

      expensesDataChart.push([
        "Costos indirectos de fabricación",
        data.expensesDescription["73"].value,
      ]);
      expensesDataChart.push([
        "Contrato de servicios",
        data.expensesDescription["74"].value,
      ]);
    }
    Highcharts.chart("chartExpensesGeneral", {
      ...
      tooltip: {
        pointFormat:
          "{series.name}: <b>{point.percentage:.1f}%</b> <br> {series.name}: <b> $ {point.y:.2f} </b>",
      },
      series: [
        {
          name: "Gastos",
    ...
    data.rawMaterials.forEach((rawMaterial) => {
      rawMaterialsDataChart.push([
        rawMaterial.material,
        (parseFloat(rawMaterial.cost) * 100) / totalMaterial,
      ]);
    });
      ...
      tooltip: {
        formatter: function () {
          return (
            ...
            "</b><br/>" +
            `Costo de materia prima:  <b> $  ${$.number(
              (this.point.y * totalMaterial) / 100,
              2,
              ",",
              "."
            )} </b> <br>
