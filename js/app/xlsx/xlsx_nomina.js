...
function verifyErrorsRosters(jsonObj, jsonProductObj) {
  ...
  for (let index = 0; index < jsonObj.length; index++) {
    ...
    if (roster.Cargo == undefined) {
      errors.push({ type: 'El cargo no puede estar vacio', row: (roster.__rowNum__ + 1) })
    }
    if (roster.Proceso != undefined) {
      if (processesInRostersJSON.filter((process) => process.name.trim().toLowerCase() == roster.Proceso.toString().trim().toLowerCase())[0] == undefined) {
        errors.push({ type: 'Este proceso no existe', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'El proceso no puede estar vacio', row: (roster.__rowNum__ + 1) })
    }
    if (roster.Cantidad != undefined) {
      if (roster.Cantidad <= 0) {
        errors.push({ type: 'La cantidad debe ser mayor a cero', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'La cantidad no puede estar vacia', row: (roster.__rowNum__ + 1) })
    }
    if (roster.Salario != undefined) {
      if (isNaN(parseFloat(roster.Salario))) {
        errors.push({ type: 'El salario debe ser un valor numérico', row: (roster.__rowNum__ + 1) })
      } else if (roster.Salario <= 0) {
        errors.push({ type: 'El salario debe ser mayor a cero', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'El salario no puede estar vacio', row: (roster.__rowNum__ + 1) })
    }
    let extrahours = roster["Horas Extras"];
    if (extrahours != undefined) {
      if (isNaN(parseFloat(extrahours))) {
        errors.push({ type: 'La horas extras debe ser un valor numérico', row: (roster.__rowNum__ + 1) })
      } else if (extrahours < 0) {
        errors.push({ type: 'Las horas extras no puede ser menores a cero', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'Las horas extras no puede estar vacias', row: (roster.__rowNum__ + 1) })
    }
    if (roster.Bonificaciones != undefined) {
      if (isNaN(parseFloat(roster.Bonificaciones))) {
        errors.push({ type: 'Las bonificaciones debe ser un valor numérico', row: (roster.__rowNum__ + 1) })
      } else if (roster.Bonificaciones < 0) {
        errors.push({ type: 'Las bonificaciones no puede ser menores a cero', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'Las bonificaciones no puede estar vacias', row: (roster.__rowNum__ + 1) })
    }

    if (roster.Dotacion != undefined) {
      if (isNaN(parseFloat(roster.Dotacion))) {
        errors.push({ type: 'Las dotación debe ser un valor numérico', row: (roster.__rowNum__ + 1) })
      } else if (roster.Dotacion < 0) {
        errors.push({ type: 'Las dotación no puede ser menor a cero', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'Las dotación no puede estar vacia', row: (roster.__rowNum__ + 1) })
    }

    if (roster.Dias != undefined) {
      if (isNaN(parseFloat(roster.Dias))) {
        errors.push({ type: 'Los días debe ser un valor numérico', row: (roster.__rowNum__ + 1) })
      } else if (roster.Dias <= 0) {
        errors.push({ type: 'Los días de trabajo debe ser mayor a cero', row: (roster.__rowNum__ + 1) })
      } else if (roster.Dias > 31) {
        errors.push({ type: 'Los días de trabajo no puede ser mayor a 31', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'Los dias de trabajo no puede estar vacia', row: (roster.__rowNum__ + 1) })
    }

    if (roster.Horas != undefined) {
      if (isNaN(parseFloat(roster.Horas))) {
        errors.push({ type: 'Las horas debe ser un valor numérico', row: (roster.__rowNum__ + 1) })
      } else if (roster.Horas <= 0) {
        errors.push({ type: 'Las horas de trabajo debe ser mayor a cero', row: (roster.__rowNum__ + 1) })
      } else if (roster.Horas > 18) {
        errors.push({ type: 'Las horas de trabajo no puede ser mayor a 18', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'Las horas de trabajo no puede estar vacia', row: (roster.__rowNum__ + 1) })
    }

    let contract = roster["Tipo de contrato"]
    if (contract != undefined) {
      if (contract != 'nomina' && contract != 'servicios') {
        errors.push({ type: 'El tipo de contrato no existe', row: (roster.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'El tipo de contrato no puede estar vacia', row: (roster.__rowNum__ + 1) })
    }
    if (roster.Prestaciones == undefined) {
      errors.push({ type: 'Las prestaciones no puede estar vacia', row: (roster.__rowNum__ + 1) })
    }else if(isNaN(parseFloat(roster.Prestaciones))){
      errors.push({ type: 'Las prestaciones debe ser numéricas', row: (roster.__rowNum__ + 1) })
    }
  }
  return errors
}
...
function uploadRosters(rosters) {
  ...
  $.post('api/upload_rosters.php', {
    ...
    if (status == 'success') {
      ...
      $.notify({
        icon: "nc-icon nc-bell-55",
        message: `Se ${countSuccess > 1 ? 'han' : 'ha'} cargado ${countSuccess} ${countSuccess > 1 ? 'nominas' : 'nomina'}`
      }, {
        type: 'success',
        timer: 8000
      })
...
function generateFileRosters() {
  ...
  $.get('api/get_rosters.php', (data, status) => {
    ...
    if (ws_data.length <= 0) {
      ...
      saveAs(new Blob([wbout], { type: "application/octet-stream" }), 'Nominas.xlsx')
