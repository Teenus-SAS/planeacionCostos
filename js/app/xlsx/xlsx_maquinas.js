$('#fileMachines').change(function () {
  ...
  reader.onloadend = function () {
    ...
    if (errorsMachines.length == 0 && workbook.Sheets['Maquinas'] != undefined) {
      ...
      $.dialog({
        title: 'Peligro',
        content: 'Este Archivo no cumple los formatos indicados <br>' + bugsToString(errorsMachines),
        type: 'red',
      });
      $('#form-maquinas')[0].reset()
      clearFile(fileInput)
...
function verifyErrorsMachines(jsonObj) {
  ...
  for (let index = 0; index < jsonObj.length; index++) {
    ...
    if (machine.Maquina == undefined) {
      errors.push({ type: 'El nombre de la máquina no puede ser vacio', row: (machine.__rowNum__ + 1) })
    }
    ...
    if (machine["Años"] == undefined) {
      ...
      errors.push({ type: 'Los años de depreciación no puede ser 0', row: (machine.__rowNum__ + 1) })
    } else if (isNaN(parseFloat(machine["Años"]))) {
      errors.push({ type: 'Los años de depreciación debe ser Numérico', row: (machine.__rowNum__ + 1) })
    }
    if (machine["Valor Residual"] == undefined) {
      errors.push({ type: 'El Valor Residual no puede ser vacio', row: (machine.__rowNum__ + 1) })
    } else if (isNaN(parseFloat(machine["Valor Residual"]))) {
      errors.push({ type: 'El valor residual debe ser Numérico', row: (machine.__rowNum__ + 1) })
    }
  }
  return errors
}
...
function uploadMachines(machines) {
  ...
  $.post('api/upload_machines.php', { machines: JSON.stringify(machines) }, (data, status) => {
    if (status == 'success') {
      let countSuccess = 0
      ...
      $.notify({
        icon: "nc-icon nc-bell-55",
        message: `Se ${countSuccess > 1 ? 'han' : 'ha'} cargado ${countSuccess} ${countSuccess > 1 ? 'maquinas' : 'maquina'}`
      }, {
        type: 'success',
        timer: 8000
      })
