...
function loadedFileGG(req) {
  if (productsJSONInExpenses != undefined) {
    ...
    if (ws_data.length <= 0) {
      saveAs('/formatos/formato-gastos-generales.xlsx', 'formato Gastos Generales.xlsx')
    } else {
      ...
      saveAs(new Blob([wbout], { type: "application/octet-stream" }), 'Gastos Generales.xlsx')
    }
...
function loadedFileUploadGG(reader, fileInput) {
  if (productsJSONInExpenses != undefined) {
    ...
    let productsExpenses = XLSX.utils.sheet_to_json(workbook.Sheets['Gastos Generales'])
    ...
    if (errorsproductsExpenses.length == 0 && workbook.Sheets['Gastos Generales'] != undefined) {
      ...
      $.dialog({
        ...
        type: 'red',
        icon: 'fas fa-warning',
        content: 'Este Archivo no cumple los formatos indicados <br>' + bugsToString(errorsproductsExpenses)
      })
      clearFile(fileInput)
    }
    completeSpinner()
...
function verifyErrorsProductsExpenses(jsonObj) {
  ...
  for (let index = 0; index < jsonObj.length; index++) {
    ...
    if (productExpenses.Referencia != undefined) {
      if (productsJSONInExpenses.filter((product) => product.ref == productExpenses.Referencia)[0] == undefined) {
        errors.push({ type: 'Este Producto no existe', row: (productExpenses.__rowNum__ + 1) })
      }
    } else {
      errors.push({ type: 'La referencia del Producto no puede ser vacia', row: (productExpenses.__rowNum__ + 1) })
    }
    if (productExpenses.unidades == undefined) {
      errors.push({ type: 'Las Unidades Vendidas no puede ser vacio', row: (productExpenses.__rowNum__ + 1) })
    }else if(isNaN(parseFloat(productExpenses.unidades))){
      errors.push({ type: 'Las Unidades Vendidas debe ser un valor numerico', row: (productExpenses.__rowNum__ + 1) })
    }
    if (productExpenses.volumen == undefined) {
      errors.push({ type: 'El Volumen de Ventas no puede ser vacio', row: (productExpenses.__rowNum__ + 1) })
    } else if (productExpenses.unidades == 0 && productExpenses.volumen > 0) {
      errors.push({ type: 'Las Unidades Vendidas no puede ser 0', row: (productExpenses.__rowNum__ + 1) })
    }else if(isNaN(parseFloat(productExpenses.volumen))){
      errors.push({ type: 'El Volumen de Ventas debe ser numérico', row: (productExpenses.__rowNum__ + 1) })
    }
  }
  return errors
}
...
function uploadProductsExpenses(productsExpenses) {
  ...
  $.post('api/upload_expenses.php', {
    ...
    if (status == 'success') {
      ...
      $.notify({
        icon: "nc-icon nc-bell-55",
        message: `Se ${countSuccess > 1 ? 'han' : 'ha'} cargado ${countSuccess} ${countSuccess > 1 ? 'Gastos Generales' : 'Gasto General'}`
      }, {
        type: 'success',
        timer: 8000
      })
