...
function verifyErrorsRawMaterials(jsonObj, jsonProductObj) {
  ...
  for (let index = 0; index < jsonObj.length; index++) {
    ...
    if (rawMaterial.Referencia != undefined) {
      ...
      errors.push({ type: 'La referencia del Producto no puede ser vacia', row: (rawMaterial.__rowNum__ + 1) })
    }
    if (rawMaterial.Material != undefined) {
      ...
      errors.push({ type: 'El material no puede ser vacio', row: (rawMaterial.__rowNum__ + 1) })
    }
...
function verifyErrorsProducts(jsonObj) {
  ...
  for (let index = 0; index < jsonObj.length; index++) {
    ...
    if (product.Referencia == undefined) {
      errors.push({ type: 'La referencia del producto no puede ser vacia', row: (product.__rowNum__ + 1) })
    }
    if (product.Producto == undefined) {
      errors.push({ type: 'El nombre del producto no puede ser vacia', row: (product.__rowNum__ + 1) })
    }
  }
  return errors
}
