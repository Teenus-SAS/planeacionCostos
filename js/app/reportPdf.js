...
$('#downloaad-pdf').click(() => {
  ...
  productsReq.forEach((productReq, indx) => {
    ...
    doc.text(`pág. ${countPages}`, doc.internal.pageSize.getWidth() - 60, countPixeles)
    countPixeles += 20
    countPages += 2
  });
  doc.addPage()
  ...
  doc.autoTable({
    head: [[{ content: 'Precio de Venta', colSpan: 2, styles: { halign: 'center' } }]],
    body: [
      [{ content: 'COP', styles: { halign: 'center' } }, { content: 'USD', styles: { halign: 'center' } }],
      [{ content: `$ ${$.number(consolidated.salePrice, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(consolidated.salePrice / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }]
    ],
  ...
  doc.autoTable({
    head: [[{ content: 'Costos Totales', colSpan: 2, styles: { halign: 'center' } }]],
    body: [
      [
      ...
      [
        { content: `$ ${$.number(consolidated.totalCost, 2, ',', '.')}`, styles: { halign: 'right' } },
        { content: `$ ${$.number(consolidated.totalCost / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }
      ]
    ],
    startY: heightautotable,
    showHead: 'firstPage',
  ...
  doc.autoTable({
    ...
    body: [
      [{ content: 'Detalles', styles: { halign: 'center' } }, { content: 'COP', styles: { halign: 'center' } }, { content: 'USD', styles: { halign: 'center' } }],
      [{ content: 'Materia Prima', styles: { halign: 'left' } }, { content: `$ ${$.number(consolidated.rawMaterialExpenses, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(consolidated.rawMaterialExpenses / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
      [{ content: 'Mano de Obra', styles: { halign: 'lef' } }, { content: `$ ${$.number(consolidated.laborCost, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(consolidated.laborCost / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
      [{ content: 'Costos Indirectos de fabricación', styles: { halign: 'left' } }, { content: `$ ${$.number(consolidated.indirectExpenses, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(consolidated.indirectExpenses / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
      [{ content: 'Total', styles: { halign: 'left' } }, { content: `$ ${$.number(consolidated.cost, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(consolidated.cost / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }]
    ],
  ...
  doc.autoTable({
    ...
    body: [
      [{ content: 'Detalles', styles: { halign: 'center' } }, { content: 'COP', styles: { halign: 'center' } }, { content: 'USD', styles: { halign: 'center' } }],
      [{ content: 'Gastos Generales', styles: { halign: 'left' } }, { content: `$ ${$.number(consolidated.generalExpenses, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(consolidated.generalExpenses / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
      [{ content: 'Comision de Ventas', styles: { halign: 'lef' } }, { content: `$ ${$.number(consolidated.salesCommission, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(consolidated.salesCommission / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
      [{ content: 'Rentabilidad', styles: { halign: 'left' } }, { content: `$ ${$.number(consolidated.profitability, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(consolidated.profitability / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }]
    ],
    startY: doc.autoTable.previous.finalY + 10,
    showHead: 'firstPage',
    styles: { overflow: 'hidden' },
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        doc.autoTable({
          ...
          body: [
            [{ content: 'COP', styles: { halign: 'center' } }, { content: 'USD', styles: { halign: 'center' } }],
            [{ content: `$ ${$.number(data.salePrice, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(data.salePrice / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }]
          ],
          startY: heightautotable,
          showHead: 'firstPage',
          styles: { overflow: 'hidden' },
        ...
        doc.autoTable({
          ...
          body: [
            ...
            [
              { content: `$ ${$.number(data.totalCost, 2, ',', '.')}`, styles: { halign: 'right' } },
              { content: `$ ${$.number(data.totalCost / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }
            ]
          ],
          startY: heightautotable,
          showHead: 'firstPage',
        ...
        doc.autoTable({
          ...
          body: [
            [{ content: 'Detalles', styles: { halign: 'center' } }, { content: 'COP', styles: { halign: 'center' } }, { content: 'USD', styles: { halign: 'center' } }],
            [{ content: 'Materia Prima', styles: { halign: 'left' } }, { content: `$ ${$.number(data.rawMaterialExpenses, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(data.rawMaterialExpenses / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
            [{ content: 'Mano de Obra', styles: { halign: 'lef' } }, { content: `$ ${$.number(data.laborCost, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(data.laborCost / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
            [{ content: 'Costos Indirectos de fabricación', styles: { halign: 'left' } }, { content: `$ ${$.number(data.indirectExpenses, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(data.indirectExpenses / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
            [{ content: 'Total', styles: { halign: 'left' } }, { content: `$ ${$.number(data.cost, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(data.cost / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }]
          ],
          startY: doc.autoTable.previous.finalY + 10,
          showHead: 'firstPage',
          styles: { overflow: 'hidden' },
        ...
        doc.autoTable({
          ...
          body: [
            [{ content: 'Detalles', styles: { halign: 'center' } }, { content: 'COP', styles: { halign: 'center' } }, { content: 'USD', styles: { halign: 'center' } }],
            [{ content: 'Gastos Generales', styles: { halign: 'left' } }, { content: `$ ${$.number(data.generalExpenses, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(data.generalExpenses / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
            [{ content: 'Comision de Ventas', styles: { halign: 'lef' } }, { content: `$ ${$.number(data.salesCommission, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(data.salesCommission / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }],
            [{ content: 'Rentabilidad', styles: { halign: 'left' } }, { content: `$ ${$.number(data.profitability, 2, ',', '.')}`, styles: { halign: 'right' } }, { content: `$ ${$.number(data.profitability / limInf, 2, ',', '.')}`, styles: { halign: 'right' } }]
          ],
          startY: doc.autoTable.previous.finalY + 10,
          showHead: 'firstPage',
          styles: { overflow: 'hidden' },
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            chartMaterialCost.options.plugins.datalabels.formatter = function (value, context) {
        ...
        doc.addImage(chartExpensesDescription.toBase64Image(), 'PNG', 30, 420, 250, 200)
        doc.addImage(chartMaterialCost.toBase64Image(), 'PNG', 300, 420, 250, 200)
        count++
            ...
            success: function (data) {
              ...
              toDataURL(logo, function (imgBase64) {
                for (let page = 1; page <= doc.getNumberOfPages(); page++) {
                  doc.setFontSize(10)
                  doc.setPage(page)
                  doc.text(doc.internal.pageSize.getWidth() - 100, doc.internal.pageSize.getHeight() - 20, `Página ${page} de ${doc.getNumberOfPages()}`); //print number bottom right
                  doc.addImage(imgBase64, 30, 15, 50, 50)
                  doc.setTextColor(24, 79, 116)
                  doc.setFontStyle('bold')
                  doc.text(data.company.tradename || data.company.bussinesReason, 90, 45)
...
/**
 * funcion para convertir a data64 una imagen
 * @param {*} url url de la imagen
 * @param {*} callback  callback que se quiere urilizar con los datos
 */
