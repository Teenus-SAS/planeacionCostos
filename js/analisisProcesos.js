  ...
  $.get(
    ...
    (_processes, status, xhr) => {
      ...
      $("#input-procesosA").change(function () {
        ...
        $("#cargaTabla").empty();
        datos = [];
        let processesSelected = _processes.filter(
          (process) => process.id == $(this).val()
        )[0];
        $("#encabezado_tabla").empty();
        $("#encabezado_tabla").append("<th>nombre</th><th>valor</th>");
        
        $.get(
          "api/get_processes.php?dataTable=true&id=" + processesSelected.id,
          (_process, status, xhr) => {
            ...
            if (processJSON != null) {
              $("#modal").hide();
              $("#cargaTabla").append(
                '<div class="col-md-10 col-sm-12 col-12 col-xs-12 mb-5"><div class="card py-2"> <h3 class="card-title bg-primary text-white text-left" style="padding:2% " id="TituloCargar">Carga</h5><table class="table" id="tableAnalisisProcesos"><thead class=text-primary id="encabezado_tabla">' +
              ...
              $("#btnModificarDatos").click(function () {
                ...
                $("#title").empty();
                $("#title").append(
                  "<p>Cargue los nuevos datos<br> digitelos</p><hr>"
                );
                $("#cargaTabla").hide();
                $("#modal").show();
                ...
                $("#mantenimiento_correctivo").val(
                  processJSON[0].mantenimiento_correctivo
                );
                $("#paradas_menores").val(processJSON[0].paradas_menores);
              });
              var i = -1;
              datos[0] = {
                nombre: "Tiempo de alistamiento",
                valor: processJSON[0].tiempo_aislamiento,
              };
              datos[1] = {
                nombre: "Tiempo de operacion",
                valor: processJSON[0].tiempo_operacion,
              };
              datos[2] = {
                nombre: "Numero de maquinas",
                valor: processJSON[0].numero_maquinas,
              };
              datos[3] = {
                nombre: "Porcentaje de rechazo",
                valor: processJSON[0].porcentaje_rechazo,
              };
              datos[4] = {
                nombre: "Numero de turnos",
                valor: processJSON[0].numero_turnos,
              };
              datos[5] = {
                nombre: "Distancia",
                valor: processJSON[0].distancia,
              };
              datos[6] = {
                nombre: "Disponibilidad",
                valor: processJSON[0].disponibilidad,
              };
              datos[7] = {
                nombre: "Mantenimiento correctivo",
                valor: processJSON[0].mantenimiento_correctivo,
              };
              datos[8] = {
                nombre: "Paradas menores",
                valor: processJSON[0].paradas_menores,
              };
              datos[8] = {
                nombre: "Tiempo  total del ciclo",
                valor:
                  parseInt(processJSON[0].tiempo_aislamiento) +
                  parseInt(processJSON[0].tiempo_operacion),
              };
            ...
            //$('#cuerpo_tabla').append("<tr><td>"+key+"</td><td>"+processJSON[key]+"</td></tr>")
              ...
              //$('#modal').modal({backdrop:'static', keyboard:false})
              $("#title").append(
                "<p>El producto no cuenta con datos para el analisis<br> digitelos</p><hr>"
              );
              $("#modal").show();
            }
          }
...
$("#form-data-process").submit(function (e) {
  ...
  let data =
    serealize +
    `&id_proceso=${$("#input-procesosA").val()}&tiempo_aislamiento=${$(
      "#tiempo_aislamiento"
    ).val()}&tiempo_operacion=${$("#tiempo_operacion").val()}` +
    `&numero_maquinas=${$("#numero_maquinas").val()}&porcentaje_rechazo=${$(
      "#porcentaje_rechazo"
    ).val()}&numero_turnos=${$("#numero_turnos").val()}` +
    `&distancia=${$("#distancia").val()}&disponibilidad=${$(
      "#disponibilidad"
    ).val()}&mantenimiento_correctivo=${$("#mantenimiento_correctivo").val()}` +
  ...
  setTimeout(2000);
  $("#cargaTabla").append(
    '<div class="col-md-10 col-sm-12 col-12 col-xs-12 mb-5"><div class="card py-2"><h5 class="card-title bg-primary text-white text-left" style="padding:2% " id="TituloCargar">Carga</h5><table class="table" id="tableAnalisisProcesos"><thead class=text-primary id="encabezado_tabla">' +
  ...
  datos[0] = {
    nombre: "Tiempo de alistamiento",
    valor: $("#tiempo_aislamiento").val(),
  };
  datos[1] = {
    nombre: "Tiempo de operacion",
    valor: $("#tiempo_operacion").val(),
  };
  datos[2] = {
    nombre: "Numero de maquinas",
    valor: $("#numero_maquinas").val(),
  };
  datos[3] = {
    nombre: "Porcentaje de rechazo",
    valor: $("#porcentaje_rechazo").val(),
  };
  ...
  datos[7] = {
    nombre: "Mantenimiento correctivo",
    valor: $("#mantenimiento_correctivo").val(),
  };
  datos[8] = { nombre: "Paradas menores", valor: $("#paradas_menores").val() };
  datos[8] = {
    nombre: "Tiempo  total del ciclo",
    valor:
      parseInt($("#tiempo_aislamiento").val()) +
      parseInt($("#tiempo_operacion").val()),
  };
...
$("#btnValidarDatos").click(function () {
  ...
  $.get(
    "api/get_processes.php?dataTable=true&id=" + $("#input-procesosA").val(),
    (_process, status, xhr) => {
      processJSON = _process.data;
      datosAnalisis[0] = {
        nombre: "distancia",
      ...
      $("#cargaTabla").hide();
      $("#cargaTabla2").append(
        '<h1>Analisis</h1> <div class ="row"><div class="col-md-6 col-sm-12 col-12 col-xs-12 mb-5">' +
      ...
      $("#btnValidarDatos2").click(function () {
        ...
        datosAnalisis[1] = {
          nombre: "veces al dia",
          valor: $("#veces_dia2").val(),
        };
        let dTotal =
          parseFloat(processJSON[0].distancia) *
          parseFloat($("#veces_dia2").val());
        datosAnalisis[2] = { nombre: "distancia total dia", valor: dTotal };
        datosAnalisis[3] = {
          nombre: "tiempo de espera",
          valor: $("#tiempo_espera2").val(),
        };
        ...
        let tTiempoMes =
          tTiempodia * (parseFloat($("#turnos_dia2").val()) * 26);
        datosAnalisis[5] = {
          nombre: "Total tiempo de espera mes",
          valor: tTiempoMes,
        };
        ...
        $("#cargaxd").empty();
        $("#cargaxd").append(
          '<div class="card py-2"><h5 class="card-title bg-primary text-white text-left" style="padding:2% " id="TituloCargar">Propuesto</h5>' +
